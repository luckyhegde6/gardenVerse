...
function GardenMapScreen() {
  ...
  const loadNearbyGardeners = async () => {
    try {
      const { data } = await axios.get('http://localhost:3001/api/v1/geo/nearby', {
      ...
      <View className="flex-1 bg-gray-200 m-4 rounded-2xl overflow-hidden">
        ...
        {gardeners.length > 0 && (
          <View className="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl p-4 max-h-48">
            <Text className="text-sm font-semibold text-gray-900 mb-2">Nearby Gardeners</Text>
            {gardeners.slice(0, 3).map((g) => (
              <TouchableOpacity key={g.id} className="flex-row items-center py-2 border-b border-gray-100">
                <View className="w-8 h-8 rounded-full bg-primary-100 items-center justify-center mr-3">
                  <Text className="text-sm">{g.displayName?.[0] || g.username[0]}</Text>
                </View>
                <View className="flex-1">
                  <Text className="text-sm font-medium text-gray-900">{g.displayName || g.username}</Text>
                ...
                <Text className="text-xs text-primary-600 font-medium">{g.sustainabilityScore} pts</Text>
              </TouchableOpacity>
            ))}
          </View>
        )}
